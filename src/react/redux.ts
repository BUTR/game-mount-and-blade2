...
extend('$auto', (value: Spec<object>, object: object) => {
  return object !== undefined && object !== null ? update(object, value) : update({}, value);
});
...
type SpecWithAuto<T> = Spec<T, AutoCommands<T>> | { [K in keyof T]?: SpecWithAuto<T[K]> | AutoCommand<T[K]> };

// Typed wrapper for update that allows $auto command
export const updateAuto = <T>(object: T, spec: SpecWithAuto<T>): T => update(object, spec as Spec<T, AutoCommands<T>>);
