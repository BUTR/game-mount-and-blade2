...
import { hasPersistentBannerlordMods, hasSessionWithBannerlord } from '../vortex';
import {
  AVAILABLE_STORES,
...
export const installModuleAsync = async (
  ...
  const resultRaw = launcherManager.installModule(filesWithFullPath, [moduleInfo]);
  ...
  const hasObfuscatedBinaries = await launcherManager.isObfuscatedAsync(moduleInfo);
