...
export const getVersion = (
  metadata: vetypes.DependentModuleMetadata,
): string => {
...
export const isVersionRangeEmpty = (
  avr: vetypes.ApplicationVersionRange,
): boolean => isVersionEmpty(avr.min) && isVersionEmpty(avr.max);

export const getVersionString = (av?: vetypes.ApplicationVersion): string =>
  av && !isVersionEmpty(av) ? ` ${versionToString(av)}` : "";

export const getVersionRangeString = (
  avr?: vetypes.ApplicationVersionRange,
): string =>
  avr && !isVersionRangeEmpty(avr)
    ? ` ${versionToString(avr.min)} - ${versionToString(avr.max)}`
    : "";
